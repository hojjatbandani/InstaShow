...
import {
	BadgeCheckIcon,
	CalenderIcon,
	CheckCircleIcon, DryWallIcon, FinalIcon, FramingIcon,
	HomeIcon,
	LocationIcon, PermitIcon,
	StarIcon,
...
const project = {
	...
	features: [
		{title: "Separate Entrance", description: "Secure side entry with fire-rated door."},
		{title: "Soundproofing", description: "R20 insulation + resilient channel ceiling."},
		{title: "Custom Kitchen", description: "Quartz countertops and soft-close cabinets."},
		{title: "Egress Windows", description: "Enlarged windows for bedroom safety."},
	],
	timeline: [
		{
			week: "Week 1-2",
			title: "Permits & Demolition",
			description: "Acquired development permit from City of Calgary. Cleared existing framing and rough-ins.",
			Icon: PermitIcon,
		},
		{
			week: "Week 3-4",
			title: "Framing & Mechanical",
			description: "Framed bedrooms and bathroom. Installed HVAC ducting and new electrical sub-panel.",
			Icon: FramingIcon,
		},
		{
			week: "Week 5-7",
			title: "Drywall & Finishes",
			description: "Insulation, poly vapour barrier, boarding, taping, and painting. Installed flooring.",
			Icon: DryWallIcon,
		},
		{
			week: "Week 8",
			title: "Final Inspection",
			description: "Passed final city inspections. Installed fixtures, appliances, and final cleanup.",
			Icon: FinalIcon,
		},
	],
	documents: [
					...
					<div className="flex flex-col lg:flex-row gap-12">
						...
						<div className="w-full lg:w-2/3">
							...
							<div className="relative min-h-[200px] md:h-[480px] rounded-xl overflow-hidden mb-4">
								...
								/>
								<span
									className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-md text-sm font-semibold text-[#0F172A]">
									After Completion
								</span>
							</div>
							{/* Gallery Thumbnails */}
							...
							<div className="w-full my-12">
								...
								{project.overview.map((paragraph, idx) => (
									<p key={idx} className="text-colors-muted text-sm md:text-base leading-relaxed mb-4">
										{paragraph}
									</p>
								))}
							</div>
							...
							<div className="w-full my-12">
								...
								<Grid gap="sm" cols={2} className="">
									{project.features.map((feature, idx) => (
										<Flex gap="md" align="start" direction="row" key={idx} className="bg-[#F1F5F9] rounded-lg p-4">
											<CheckCircleIcon size="lg" className="text-[#0066CC] flex-shrink-0 mt-0.5"/>
											<Flex gap="sm" align="start" direction="col">
												<h3 className="font-semibold text-[#0F172A] text-sm">{feature.title}</h3>
												<p className="text-colors-muted text-[13px]">{feature.description}</p>
											</Flex>
										</Flex>
									))}
								</Grid>
									...
									{project.timeline.map((item, idx) => (
										<div key={idx} className="flex gap-4">
											<div className="flex flex-col items-center">
												<div
													className="p-2 rounded-full bg-[#FFB020] flex items-center justify-center text-[#3B82F6]">
													<item.Icon size="lg"/>
												</div>
											...
											<div className="flex-1 pb-10">
												...
												<h3 className="font-semibold text-[#0B1B2B] mb-1">{item.title}</h3>
												<p className="text-colors-muted text-sm">{item.description}</p>
											</div>
										</div>
									))}
								</div>
									...
									{project.documents.map((doc, idx) => (
										...
										>
											...
											<div className="text-left">
												<p className="text-sm font-medium text-[#0B1B2B]">{doc.name}</p>
												<p className="text-xs text-colors-muted">{doc.size}</p>
											</div>
											<svg className="w-5 h-5 text-colors-muted ml-2" fill="none" viewBox="0 0 24 24"
													 stroke="currentColor">
						...
						<div className="w-full lg:w-1/3 space-y-6">
							...
							<div className="rounded-xl border border-gray-200 p-5 md:p-6">
								<div className="flex flex-col items-center text-center mb-4">
									...
									/>
									<h3 className="font-semibold text-[#0F172A] text-lg">{project.contractor.name}</h3>
									<div className="flex items-center gap-2 mt-2">
										{project.contractor.verified && (
											<div className="flex items-center bg-[#FFB020] px-2 py-2 rounded-full">
												<BadgeCheckIcon size="md" className="text-[#0066CC]"/><span
												className=" text-[#0066CC] text-xs">Verified Pro</span>
											</div>
										)}

										<div className="flex items-center bg-[#DCFCE7] px-2 py-2 rounded-full gap-1">
											<StarIcon size="sm" className="text-[#166534]"/>
											<span className="text-[#166534] text-xs">{project.contractor.rating}</span>
											<span className="text-[#166534] text-xs">({project.contractor.reviews})</span>
										</div>
									</div>
								</div>
							...
							{/* Project Details */}
							<div className="rounded-xl border border-gray-200 p-5 md:p-6">
								<h3 className="font-semibold text-[#0B1B2B] mb-4">Project Details</h3>
								<div className="space-y-5">
									{Object.entries({
										...
										<div key={label}
											...
											<span className="font-medium text-[#0B1B2B]">{value}</span>
										</div>
									))}
								</div>
							</div>
							...
							{/* Inspired CTA */}
							<div
								className="bg-gradient-to-r from-btn-primary-from to-btn-primary-to rounded-xl p-5 md:p-6 text-white">
								<h3 className="font-semibold mb-2">Inspired by this?</h3>
								<p className="text-white/80 text-sm mb-4">
									Get accurate quotes from top-rated local pros for your own basement renovation.
								</p>
								<Button
									variant="none"
									size="md"
